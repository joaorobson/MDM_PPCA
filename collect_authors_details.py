...
URLS_CAMARA_AUTORES_DETALHES = {}
async_collector = AsyncCollector()


""" directory_path = Path("autores_pls_camara")
for file_path in directory_path.iterdir():
    if file_path.is_file():
        content = json.loads(file_path.read_text())
        if content:
            print(content, file_path)
            new_path = file_path.as_posix().replace('autores_pls_camara', 'detalhes_autores_pls_camara')
            print(new_path)
            urls = [author['uri'] for author in content]
            URLS_CAMARA_AUTORES_DETALHES[new_path] = urls

json.dump(URLS_CAMARA_AUTORES_DETALHES, open("urls_camara_autores_detalhes.json", "w")) """


URLS_CAMARA_AUTORES_DETALHES = json.load(open("urls_camara_autores_detalhes.json", "r"))
async def collect_pls_async():
    ...
    results = {}
    for i, batch in enumerate(chunk_dict(URLS_CAMARA_AUTORES_DETALHES, 40), 1):
