...
for file_path in directory_path.iterdir():
    if file_path.is_file():
        ...
        if content:
            id_ = content[0]['id']
            url = content[0]['uriAutores']
            URLS_CAMARA_AUTORES[f"autores_pls_camara/{id_}.json"] = [url]

json.dump(URLS_CAMARA_AUTORES, open("urls_camara_autores.json", "w"))

 """
URLS_CAMARA_AUTORES = json.load(open("urls_camara_autores.json", "r"))
async def collect_pls_async():
    ...
    results = {}
    for i, batch in enumerate(chunk_dict(URLS_CAMARA_AUTORES, 40), 1):
